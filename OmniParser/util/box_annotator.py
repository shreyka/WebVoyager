...
class BoxAnnotator:
    ...
    def annotate(
        ...
        for i in range(len(detections)):
            ...
            color = Color.BLACK  # set to cv2 black
            # cv2.rectangle(
            #     img=scene,
            #     pt1=(x1, y1),
            #     pt2=(x2, y2),
            ...
            # )
            line_type = cv2.LINE_4  # Use 4-connected line for dotted effect
            for y in range(y1, y2, 2 * self.thickness):
                cv2.line(scene, (x1, y), (x1, min(y + self.thickness, y2)), color.as_bgr(), self.thickness, line_type)
                cv2.line(scene, (x2, y), (x2, min(y + self.thickness, y2)), color.as_bgr(), self.thickness, line_type)
            for x in range(x1, x2, 2 * self.thickness):
                cv2.line(scene, (x, y1), (min(x + self.thickness, x2), y1), color.as_bgr(), self.thickness, line_type)
                cv2.line(scene, (x, y2), (min(x + self.thickness, x2), y2), color.as_bgr(), self.thickness, line_type)
            if skip_label:
                continue
