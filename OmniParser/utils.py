...
def get_element_rect_from_image(image_path: str, fix_color: bool = True) -> tuple[list, list, str]:
    """
    Get interactive elements from an image using OmniParser detection.
    Similar to get_web_element_rect but without Selenium dependencies.
    
    Args:
        image_path: Path to screenshot
        fix_color: Whether to use fixed color for annotations (default: True)
    
    Returns:
        tuple[list, list, str]: (rectangles, elements, formatted_text)
        - rectangles: List of rectangle overlay elements
        - elements: List of detected elements
        - formatted_text: Tab-separated string of element descriptions
    """
    print(f"Processing image for element detection: {image_path}")
    ...
    # Get YOLO model
    model = get_yolo_model(model_path=config['som_model_path'])
    ...
    # Get annotated image and element information
    base64_annotated_image, label_coordinates, parsed_content_list = get_som_labeled_img(
        image_path,
        model,
        BOX_TRESHOLD=config['BOX_TRESHOLD'],
        output_coord_in_ratio=False,
        ocr_bbox=ocr_bbox,
        draw_bbox_config=config['draw_bbox_config'],
        caption_model_processor=None,
        ocr_text=text,
        use_local_semantics=False
    )
    
    # Format elements similar to get_web_element_rect output
    rectangles = []
    elements = []
    formatted_texts = []
    ...
    for idx, (text_content, bbox) in enumerate(zip(text, ocr_bbox)):
        if text_content.strip():
            formatted_texts.append(f"[{idx}]: \"{text_content}\"")
            elements.append({
                'type': 'text',
                'text': text_content,
                'bbox': bbox
    ...
    for idx, (label, coords) in enumerate(label_coordinates.items(), start=len(text)):
        if idx < len(parsed_content_list):
            ...
            formatted_texts.append(f"[{idx}]: <element> \"{content}\"")
    
    formatted_text = '\t'.join(formatted_texts)
    print(f"Found {len(elements)} interactive elements")
    
    return base64_annotated_image, rectangles, elements, formatted_text
